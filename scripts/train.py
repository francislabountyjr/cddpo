...
def main(_):
    ...
    pipeline = StableDiffusionPipeline.from_pretrained(
        config.pretrained.model, torch_dtype=torch.float16
    )
        ...
        for i in tqdm(
            ...
            for each_combination in combinations_list:
                ...
                for j in tqdm(
                    ...
                    if config.use_cddpo:
                        mse_loss_0 = F.mse_loss(
                            noise_pred_0.float(),
                            sample_0["next_latents"][:, j].float(),
                            reduction="mean",
                        )
                        logits = total_prob_0 - total_prob_1
                        contrastive_losses = -F.logsigmoid(config.beta * logits)
                        loss = contrastive_losses.mean() + mse_loss_0
                    else:
